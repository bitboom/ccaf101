...
- 최신 테스트를 다시 실행함
- 현재 요구사항을 다시 확인함
- scratchpad / manifest를 기준으로 상태를 재동기화함
- 요약 대신 원문 source를 다시 확인함
...
- 중간 결과를 external state에 남긴다
- checkpoint마다 방향을 다시 맞춘다
- 불확실성이 커지면 handoff를 고려한다
- summary만 믿지 않고 source로 돌아갈 수 있다
