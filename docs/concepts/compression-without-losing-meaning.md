...
> 여기서는 문맥의 수명주기 일반론보다, **문맥을 줄일 때 어떻게 다음 판단에 필요한 의미를 보존할 것인가**를 다룬다. 즉 compression을 단순 요약이 아니라, **semantic preservation 문제**로 이해하는 데 초점을 둔다.

## 왜 compression이 필요한가

장기 실행이나 긴 대화, 큰 조사 작업에서는 문맥이 빠르게 비대해진다.  
모든 것을 원문 그대로 계속 유지하려 하면:

- attention budget이 낭비되고
- 중요한 정보가 묻히고
- 후속 단계가 느려지고
- 현재 판단에 직접 필요하지 않은 세부사항이 계속 남게 된다

이럴 때 compression이 필요하다.
