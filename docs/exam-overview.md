...
- JSON 스키마, few-shot 예시, 추출 패턴을 활용한 구조화된 출력을 위한 prompt 설계
...
- "few-shot 예시를 추가하여 도구 호출 순서를 강제한다" → few-shot 자체는 유효한 기법이지만, **순서 보장은 규정 준수(compliance) 문제이므로 프로그래밍적 강제가 필요하다.**
...
| 5 | **CI/CD를 위한 Claude Code** | CI/CD 파이프라인에 Claude Code를 통합하여 자동 코드 리뷰, 테스트 케이스 생성, PR 피드백 제공. 오탐을 최소화하는 prompt 설계. | D3, D4 |
| 6 | **구조화된 데이터 추출** | 비정형 문서에서 정보를 추출하고 JSON 스키마로 검증. 엣지 케이스 처리. 다운스트림 시스템과의 통합. | D4, D5 |

### 시나리오-도메인 교차 맵

이 교차 관계를 시각적으로 정리하면 학습 우선순위를 잡는 데 도움이 된다.
...
| 도메인 | 비중 | 태스크 수 | 핵심 키워드 |
|--------|------|----------|-----------|
| **1. 에이전트 아키텍처 및 오케스트레이션** | **27%** | 7개 (1.1~1.7) | `stop_reason`, 허브-앤-스포크, Task 도구, 훅, prompt 체이닝, `fork_session` |
...
| **4. Prompt 엔지니어링 및 구조화된 출력** | **20%** | 6개 (4.1~4.6) | 명시적 기준, few-shot, nullable 필드, 검증 루프, batch API, 멀티패스 리뷰 |
...
### 사고 모델 1: 프로그래밍적 강제 > Prompt 기반 안내

**이 시험에서 가장 빈번하게 출제되는 단일 개념이다.**

핵심은 간단하다: 비즈니스 규칙이 반드시 지켜져야 하는 상황(금융 거래, 신원 확인, 정책 준수)에서 prompt 지시만으로는 충분하지 않다. Prompt는 확률적(probabilistic)이다 — 대부분의 경우 작동하지만, **비영(non-zero) 실패율**을 가진다. 반면 훅(hook)과 사전 조건 게이트(prerequisite gate)는 결정론적(deterministic)이다 — 100% 보장된다.
...
이런 문제에서 "시스템 prompt에 고객 확인이 필수라고 명시한다"나 "few-shot 예시를 추가한다"는 선택지가 그럴듯해 보이지만, 정답은 **`get_customer`가 검증된 고객 ID를 반환할 때까지 `lookup_order`와 `process_refund`를 프로그래밍적으로 차단하는 것**이다.

> **판별 공식**: "이 규칙이 지켜지지 않으면 금전적·법적 결과가 발생하는가?" → 예 → 프로그래밍적 강제. 아니오 → prompt 안내로 충분.

### 사고 모델 2: 도구 설명이 라우팅의 핵심이다

모델이 여러 도구 중 하나를 선택할 때, 그 결정을 주도하는 것은 시스템 prompt도 함수 이름도 아니다 — **도구 설명(tool description)**이다.
...
이 원칙의 실무적 함의는 명확하다: 종합(synthesis) 에이전트가 웹 검색 에이전트의 결과를 필요로 한다면, 해당 결과를 **종합 에이전트의 prompt에 직접 포함**시켜야 한다. 메타데이터(출처 URL, 문서명, 페이지 번호)와 본문 내용을 구조화된 형식으로 분리하여 전달하는 것이 모범 사례다.
