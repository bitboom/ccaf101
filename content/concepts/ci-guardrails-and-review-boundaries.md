...
- 위험한 파일 수정 금지
- 특정 디렉토리 변경 시 추가 검증
- 파괴적 명령 전 확인 ([[concepts/hooks-and-enforcement|hooks]]로 구현 가능)
- 테스트 실패 시 다음 단계 차단
- 중요한 작업 전 인간 승인 요구 ([[concepts/escalation-and-handoff-boundaries|escalation]])
...
### mergeable state
- 필요한 자동 검증을 통과함
- [[concepts/team-conventions-and-reproducibility|팀 규약]]과 저장소 규칙에 맞음
- 리뷰 가능한 diff를 가짐
...
그보다:
- [[concepts/validation-repair-and-retry-loops|자동 검증 루프]]를 거치게 하고
- 실패 시 되돌아가게 하고
- [[concepts/tool-safety-boundaries|위험 작업을 차단]]하고
- 인간 리뷰가 필요한 지점을 분리한다
